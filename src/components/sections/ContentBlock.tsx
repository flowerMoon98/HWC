      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div
          className={cn(
            ...
            hasImage
              ? "grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 lg:gap-24"
              : "max-w-3xl mx-auto text-center", // Use grid for image+text, simpler layout for text only
          )}
        >
          ...
          >
            ...
            <div className="prose prose-lg text-gray-600 max-w-none">
              {" "}
              {/* Basic prose styling for text */}
              {typeof text === "string" ? <p>{text}</p> : text}
            </div>
            {/* Optional CTA Button */}
            {ctaButton && (
              <div
                className={cn("mt-8", !hasImage ? "flex justify-center" : "")}
              >
                {" "}
                {/* Center button if text-only */}
                <Button
