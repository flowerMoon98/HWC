      ...
      <div className="flex flex-col w-full gap-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between w-full gap-6 md:gap-10">
            ...
            <p className="font-light text-[var(--color-neutralstaupe-greyneutral-5)] text-xs tracking-tight leading-3 w-full">
              *Protecting your privacy is our{" "}
              <AnimatedUnderlineLink
                href="/privacy-policy"
                className="font-medium text-xs leading-3 tracking-tight text-[var(--color-neutralstaupe-greyneutral-5)]"
              >
                policy
              </AnimatedUnderlineLink>
          ...
          <form
            ref={formRef}
            action={formAction}
            className="flex w-full md:w-auto items-end gap-4"
          >
              ...
              <div className="flex flex-col gap-2 mt-5">
                {companyLinks.map((link) => (
                  <AnimatedUnderlineLink
                    key={link.title}
                    href={link.href}
                    className="text-base font-light text-[var(--color-primaryseasalt)] tracking-tight leading-[21px]"
                  >
                    {link.title}
                  </AnimatedUnderlineLink>
                ))}
          ...
          <div className="flex gap-6 order-2 sm:order-1">
            {legalLinks.map((link) => (
              <AnimatedUnderlineLink
                key={link.title}
                href={link.href}
                className="text-sm font-light text-[var(--color-primaryseasalt)] tracking-tight leading-[17px]"
              >
                {link.title}
              </AnimatedUnderlineLink>
            ))}
